...
const EbookViewer = () => {
  ...
  }, [currentPage, flipping, isFullscreen]);
  ...
  const renderBookContent = () => (
    <div className={`relative w-full ${isFullscreen ? "max-w-5xl" : "max-w-2xl"}`}>
      ...
      <div className={`relative flex justify-center items-center w-full aspect-[2/1.2] mb-10 perspective-1000 ${isFullscreen ? "scale-110" : ""}`}>
        {/* Book binding */}
        ...
        <div className="w-full h-full flex relative book-shadow rounded-lg overflow-hidden">
          {/* Left page */}
          <div
            className={`w-1/2 h-full bg-white dark:bg-gray-800 relative origin-right transform transition-all duration-800 ease-in-out ${flipping && direction === "backward" ? "animate-page-flip-right" : ""}`}
            style={{
          ...
          >
            <div className="absolute inset-0 p-4 flex justify-center items-center">
              <img src={pages[currentPage]} alt={`Page ${currentPage + 1}`} className="w-full h-full object-contain" />
            </div>
          ...
          {/* Right page */}
          <div
            className={`w-1/2 h-full bg-white dark:bg-gray-800 relative origin-left transform transition-all duration-800 ease-in-out ${flipping && direction === "forward" ? "animate-page-flip-left" : ""}`}
            style={{
              boxShadow: "inset 10px 0 20px rgba(0,0,0,0.1)",
              transformStyle: "preserve-3d",
              backfaceVisibility: "hidden",
          ...
          >
            {currentPage + 1 < pages.length && (
              <div className="absolute inset-0 p-4 flex justify-center items-center">
                <img src={pages[currentPage + 1]} alt={`Page ${currentPage + 2}`} className="w-full h-full object-contain" />
              </div>
            )}
      ...
      <div className="flex justify-center items-center mt-2 mb-6 space-x-8">
        ...
        <button
          ...
          className="p-3 rounded-full bg-black dark:bg-white text-white dark:text-black dark:hover:text-white hover:bg-blue-600 hover:scale-105 shadow-md transition-all"
        >
        ...
        <button
          ...
          className={`p-3 rounded-full transition-all ${
            currentPage >= pages.length - 2 || flipping ? "opacity-50 cursor-not-allowed bg-gray-400" : "bg-black dark:bg-white text-white dark:text-black dark:hover:text-white hover:bg-blue-600 hover:scale-105 shadow-md"
          }`}
        >
  ...
  if (isFullscreen) {
    return (
      <div className="fixed inset-0 z-50 bg-gray-100 dark:bg-gray-900 text-black dark:text-white flex items-center justify-center">
        {renderBookContent()}
      </div>
    );
  }
